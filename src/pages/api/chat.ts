...
2. **agent** — A portable AI agent definition: personality, capabilities, system prompt additions, model preferences, memory references. Agents can be exported and imported across environments.
...
5. **credential** — Verifiable claims issued by a third party about your identity (e.g., "this person completed X" or "this person is authorized for Y").
6. **context** — Project planning state: vision, decisions, conventions, milestones, debates. Portable planning as structured data.
...
2. **SHA-256 hash** the canonical JSON → this becomes \`content_hash\`
3. **Sign** the UTF-8 bytes of the content_hash hex string with the author's Ed25519 private key → this becomes \`signature\`
...
When an agent is exported as an .opkg, its profile, personality, and memory travel with it — but its trust level does NOT. An agent that was Level 3 (Collaborator) in one environment arrives at Level 0 (Observer) in a new environment and must earn trust again from scratch. This is a security design decision: trust is contextual, earned, and non-transferable.

### Transmission Classes

Packages carry a transmission_class hint:
- **local** — No size constraints. Full fidelity. On-device use.
- **network** — Compression recommended. Standard internet transfer.
- **interplanetary** — Maximally compressed, fully self-contained, no external references. Designed for scenarios where round-trip latency is 4–24 minutes and bandwidth is rationed. This is the Phase 2 bridge: the same format that powers your AI on Earth is what ships to Mars.

### Design Principles

1. **Self-sovereign** — No registration, no central authority. A keypair is all you need.
2. **Self-verifying** — Every package includes its author's DID and cryptographic signature. Verify without contacting a third party.
3. **Content-addressed** — Identified by SHA-256 hash. Immutable by design. If the content changes, the hash changes.
4. **Human-readable** — UTF-8 JSON. You can open it in a text editor and understand what you're looking at.
5. **Minimal core, extensible** — The standard defines the minimum. Implementation-specific data goes in namespaced extensions without polluting the core spec.
6. **Trust is contextual** — Permissions are earned in-environment, never exported. Security is structural, not configurable.

### What Exists Today

- Specification is **complete** across all four documents (identity, data package, agent portability, project context)
- **5 reference packages** built and tested (minimal identity, agent export, project context, workspace bundle, and more)
- **All tests passing** — the standard has its own test suite
- The spec is designed for public release at openpeople.ai — not yet published, but spec-complete and ready

### The Strategic Position

The open-people standard is to personal AI identity what:
- **RSS** was to content syndication — an open format that enabled an ecosystem
- **OAuth** was to web authentication — a standard that made interoperability possible
- **SMTP** was to email — a protocol that no single company owned

First-mover advantage on an open standard is real: you define the defaults that everyone else has to be compatible with. Even if competitors adopt the format, the first implementation that built the tooling, the reference packages, and the user base has structural advantage.
...
"What is the open-people standard?" / "How does data portability work?" / "What is .opkg?" / "How does identity work?" → Go deep on the standard. Explain Ed25519 DID, the .opkg envelope, the seven content types, and why it matters strategically. Use the STANDARD ARCHITECTURE diagram. This is a core differentiator — treat it with the same depth you'd give the trust model or the data flywheel.

"Why does the data standard matter?" / "What makes this not just another format?" → Connect to the strategic position: RSS/OAuth/SMTP analogy. Explain that the standard is what makes the data permanently valuable and the moat real. First-mover on the format = defining the defaults the ecosystem builds around.

"Can I take my data somewhere else?" / "What about vendor lock-in?" → Lead with the self-sovereign design: no registration, cryptographic ownership, human-readable JSON, any tool can verify. The standard is designed so that even if Open People Inc. disappeared tomorrow, your data would still be readable, verifiable, and useful.
